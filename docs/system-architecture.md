...
- `src/background/provider-service.js`: calls the configured OpenAI-compatible provider.
- `src/background/storage-service.js`: stores local article-library records.
- `src/side-panel/side-panel.html`: main reader surface.
- `src/side-panel/side-panel.js`: side panel controller.
- `src/side-panel/side-panel-actions.js`: side panel action handlers.
- `src/side-panel/article-library.js`: renders analyzed article history.
- `src/side-panel/export-service.js`: exports saved items to CSV.
...
7. Review, library, known-word state, and exports remain local in `chrome.storage.local`.
